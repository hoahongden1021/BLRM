...
**CANDIDATE speed fix, awaiting real-emulator test:** the watcher now resizes the actual image passed to OCR to a max side of 960 (typically 960x431) and sets `Det.limit_type=max` so RapidOCR does not upscale its short side back to 736. It then maps OCR boxes back to the stream and native ADB coordinate spaces. The CLI parameter is `start --ocr-max-size N` (`--ocr-det-max-size` remains an alias); `0` disables resizing for comparison. An UNKNOWN_SCREEN result triggers one full-resolution OCR fallback on that frame for readable diagnostics. Unchanged screens get an idle OCR refresh after one second by default. `status --brief` includes the OCR input size, fallback flag, time, frame age, classification, and decoded frame count. The actual speed and recognition of this new method still need real-emulator evidence. Keep the server console open; query status from a separate PowerShell window if the launch window has focus.
