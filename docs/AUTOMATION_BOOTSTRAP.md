...
**Further live observation:** after the second `登录游戏` tap exposed UNKNOWN_SCREEN at watcher frame 2749, a later read of `runtime/agent/screen_state.json` again classified `LOGIN_OR_ENTRY / ACCOUNT_LOGIN` with the login text and account fields. This establishes the later *observed* screen but does not show exactly when the app returned or whether a login packet was sent during that attempt. The normal `--watch` login command now watches the entire 20-second window and includes `screen_events`, `returned_to_initial_state`, `verified_transition_observed`, and sanitized `packet_headers` in `AFTER`. Exit 0 requires the final state to be a verified different screen; a transient UNKNOWN_SCREEN followed by ACCOUNT_LOGIN exits 3. `--trace-login` reports the same final-state distinction. The updated return-state reporting was checked offline with a transient UNKNOWN_SCREEN followed by ACCOUNT_LOGIN; it still needs a live emulator run. Read the newest per-connection port 19000/29000 packet headers and `CLOSED ... reason=` to identify where login failed before changing server reply formats.
