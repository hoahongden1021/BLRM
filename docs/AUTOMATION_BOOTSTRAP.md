...
**VERIFIED from real-emulator traces (2026-09-23):** one `--trace-login` tap at `(1410,550)` left ACCOUNT_LOGIN on screen for the full 20-second trace, emitted no packet headers, and did not change the Android PID. In a later run, after the START_MENU tap reached ACCOUNT_LOGIN at watcher frame 2559, one `登录游戏` tap had `transition_observed=false` at frame 2674; a second tap at the same `(1410,547)` reported `transition_observed=true` at frame 2749 but classified the next screen as UNKNOWN_SCREEN. The user's observation is that UI actions can take roughly 3–5 seconds after a CLI tap. The earlier two START_MENU state mismatch errors correctly refused to tap while OCR still showed ACCOUNT_LOGIN. GameUi's decompiled `pointEvent(II)` confirms that tapping a newly focused UI id only selects it (`actionType=-1`), while tapping the same id again activates it (`actionType=0`). These facts support two independent delays: game/UI response and the focus-then-activate interaction; UNKNOWN_SCREEN does **not** yet prove the next page is the role list.

**Further live observation:** after the second `登录游戏` tap exposed UNKNOWN_SCREEN at watcher frame 2749, a later read of `runtime/agent/screen_state.json` again classified `LOGIN_OR_ENTRY / ACCOUNT_LOGIN` with the login text and account fields. This shows what the cached watcher JSON contained at that read; the user did not include watcher running status or frame age, so whether it reflected the live screen and when any return happened are still unverified. It also does not show whether a login packet was sent during that attempt. The normal `--watch` login command now watches the entire 20-second window and includes `screen_events`, `returned_to_initial_state`, `verified_transition_observed`, and sanitized `packet_headers` in `AFTER`. Exit 0 requires the final state to be a verified different screen; a transient UNKNOWN_SCREEN followed by ACCOUNT_LOGIN exits 3. `--trace-login` reports the same final-state distinction. The updated return-state reporting was checked offline with a transient UNKNOWN_SCREEN followed by ACCOUNT_LOGIN; it still needs a live emulator run. Read the newest per-connection port 19000/29000 packet headers and `CLOSED ... reason=` to identify where login failed before changing server reply formats.

`truthan_ocr_tap.py` now waits at least five seconds before returning a verified watched transition and allows up to 20 seconds for a slow screen. For the exact watched ACCOUNT_LOGIN `登录游戏` item, it waits for a newly captured OCR frame at least five seconds after the first tap. Only if that frame still shows the same button in the same place and no server packet arrived will it make one confirmation tap. It prints `FOCUS_CONFIRM` to show whether and why it sent that second tap. If there is no post-tap OCR frame, it stops without another tap. This behavior passed offline checks for an unchanged account screen, changed screen, old frame, and already-started auth; **its automatic confirmation has not yet been verified on the emulator**. Trace output still reports screen transitions, connection changes, frame headers and sanitized logcat for 20 seconds; inspect the classification and packet headers before concluding role selection or game entry.
