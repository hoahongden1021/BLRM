...
def get_obj_image_refs(object_data_id):
    ...
    """
    data, source = get_local_resource(5, int(object_data_id))
    if data is None:
        return None, source
    if len(data) < 3:
...
def npc_view_body(sprite_id, name, level, object_data_id, x, y,
                  ...
                  auto_control_type=0, sprite_state=0, flags=b"",
                  trigger_event=0, image_ids=(), action_type=0, action=0):
    """
    ...
    b += struct.pack(">bbbb", 2, 0, 0, 0)  # type=GNPC, exType, specialEffect, remove=0

    # GNPC branch
    b += struct.pack(">hh", int(x), int(y))
    b += struct.pack(">b", int(auto_control_type))
    b += struct.pack(">b", int(sprite_state))
    ...
    b += struct.pack(">bbb", int(action_type), int(sprite_state), int(action))
                ...
                try:
                    if port == 29000:
                        ...
                        if cmd == LOGIN_GAME:
                            ...
                            time.sleep(0.15)
                            send_starter_probe_entities(c, f, port, sid, sess)
